      ...
      <div className="flex-1 overflow-y-auto bg-black/40 p-6 font-mono text-[11px] leading-relaxed">
        ...
        {events.map((e, i) => (
          <div key={i} className="flex gap-4 py-0.5">
            ...
            <span className="w-40 shrink-0 uppercase tracking-widest text-foreground">
              {e.action}
            </span>
            <span className="text-muted-foreground">→ {e.target}</span>
            <span
              className={`ml-auto uppercase tracking-widest ${e.level === "error" ? "text-destructive" : e.level === "warn" ? "text-[var(--color-chart-5)]" : "text-muted-foreground"}`}
            >
              {e.level}
            </span>
          </div>
