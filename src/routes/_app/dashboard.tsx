      ...
      <div className="flex-1 overflow-y-auto px-8 py-6">
        <div className="grid gap-4 md:grid-cols-4">
          <Metric
            ...
            value={`${online}/${workers.length}`}
            hint="Fleet health nominal"
            tone="success"
          />
          <Metric label="Active Runs" value={String(active)} hint="Includes paused approvals" />
          <Metric
            label="Workflows"
            value={String(workflows.filter((w) => w.status === "active").length)}
            hint={`${workflows.length} total defined`}
          />
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <div className="font-mono text-[11px] text-muted-foreground">
                          {r.id} · {r.triggered_by} · {r.started_at}
                        </div>
                      </div>
                    </div>
