  ...
  return (
    <>
      ...
      <div className="flex-1 space-y-6 overflow-y-auto px-8 py-6">
        <section>
          <div className="mb-3 font-mono text-[10px] uppercase tracking-widest text-muted-foreground">Users</div>
          <DataTable
            rows={users}
            rowKey={(u) => u.id}
            columns={[
              ...
              { key: "role", header: "Role", render: (u) => <span className="font-mono text-[10px] uppercase tracking-widest">{u.role}</span> },
              { key: "status", header: "Status", render: (u) => <StatusPill tone={u.status === "active" ? "success" : "warning"}>{u.status}</StatusPill> },
            ]}
          />
        </section>
        <section>
          <div className="mb-3 flex items-center justify-between">
            <div className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">API Tokens</div>
          ...
          <DataTable
            ...
            rowKey={(t) => t.id}
            columns={[
              { key: "id", header: "ID", render: (t) => <span className="font-mono text-xs text-primary">{t.id}</span> },
              { key: "label", header: "Label" },
              { key: "scope", header: "Scope", render: (t) => <span className="font-mono text-[10px] uppercase tracking-widest">{t.scope}</span> },
