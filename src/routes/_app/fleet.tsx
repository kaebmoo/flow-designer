      ...
      <PageHeader
        ...
        actions={
          <>
            ...
            <button onClick={() => setOpen(true)} className="inline-flex items-center gap-2 rounded bg-primary px-4 py-1.5 text-xs font-bold uppercase tracking-wider text-primary-foreground hover:opacity-90">
      ...
      <div className="flex-1 overflow-y-auto px-8 py-6">
        <DataTable
          ...
          columns={[
            ...
            { key: "role", header: "Role", render: (w) => <span className="font-mono text-xs">{w.role}</span> },
            { key: "tags", header: "Tags", render: (w) => (
              <div className="flex flex-wrap gap-1">
                {w.tags.map((t) => <span key={t} className="rounded border border-border bg-white/5 px-1.5 py-0.5 font-mono text-[10px]">{t}</span>)}
              </div>
            )},
            { key: "workspaces", header: "Workspaces", render: (w) => <span className="font-mono text-xs text-muted-foreground">{w.workspaces.join(", ") || "—"}</span> },
            { key: "version", header: "Version", render: (w) => <span className="font-mono text-xs text-muted-foreground">{w.version}</span> },
            ...
            { key: "last_seen", header: "Last Seen", className: "text-right", render: (w) => <span className="font-mono text-xs text-muted-foreground">{w.last_seen}</span> },
          ]}
        />
      </div>
      ...
      {open && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/60 p-4 backdrop-blur-sm animate-fade-in">
          <div className="w-full max-w-md rounded-lg border border-border bg-card p-6 animate-scale-in">
            ...
            <p className="mt-1 text-xs text-muted-foreground">Atlas will poll `/healthz` and `/v1/agent/info` after save.</p>
            <div className="mt-5 space-y-4">
              {[
                ...
                <label key={f.k} className="block">
                  <span className="mb-1 block font-mono text-[10px] uppercase tracking-widest text-muted-foreground">{f.label}</span>
                  <input
            ...
            <div className="mt-6 flex justify-end gap-2">
              <button onClick={() => setOpen(false)} className="rounded border border-border px-3 py-1.5 text-xs">Cancel</button>
              <button
                onClick={() => {
                  if (!form.name || !form.base_url) return;
                  addWorker({ name: form.name, base_url: form.base_url, role: form.role, tags: form.tags.split(",").map((s) => s.trim()).filter(Boolean), workspaces: [], version: "1.4.2" });
                  setOpen(false);
