...
- **If Cloudflare Workers is chosen instead,** treat it as a *distinct* deployment runtime (Workers/`workerd`, not Node): validate the session primitive, streaming/SSE proxy, and any Node-API usage against the Workers runtime separately before committing.
...
- **Do not implement custom two-key rotation** unless the framework primitive is insufficient *and* the user approves the added complexity. Rotate on a schedule and on suspected compromise (accepting that active sessions must re-authenticate).
